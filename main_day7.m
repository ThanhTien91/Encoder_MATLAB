% ==========================================================================
% FILE: main_day7.m
% MODULE: Day 7 - Dynamic Scenario Testing (5 Edge Cases)
% DESCRIPTION: Evaluates 5 dynamic scenarios (Low Speed, Zero-Crossing,
%              Fast Accel, Hard Brake, Micro-Vibrations) with full
%              quantitative metrics (RMSE, MAE, Max, Latency, Timeout).
% ==========================================================================
...
fprintf('======================================================================\n');
fprintf('    ENCODER PROJECT - DAY 7: DYNAMIC SCENARIO TESTING\n');
fprintf('======================================================================\n\n');
...
for i = 1:5
    ...
    % Latency (Cross-Correlation)
    try
        delay_samples = finddelay(omega_ref, omega_Hybrid);
    catch
        [c, lags] = xcorr(omega_ref - mean(omega_ref), ...
            omega_Hybrid - mean(omega_Hybrid));
        [~, I] = max(abs(c));
    ...
    fprintf('----------------------------------------------------------------------\n');
    fprintf('=> System Latency (Hybrid): %.2f ms\n', latency_ms);
    fprintf('=> T-Method Timeouts      : %d events\n\n', timeout_events);
    
    %% ============================================================
    % VISUALIZATION
    %% ============================================================
    
    fig = figure('Name', titles{i}, 'NumberTitle', 'off', ...
