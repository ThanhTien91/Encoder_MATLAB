% ==========================================================================
% FILE: run_sensitivity.m
% MODULE: Sensitivity Analysis (Pulse Loss Sweep)
% DESCRIPTION: Evaluates position compensator RMSE & Final Drift
%              across pulse loss rates (0.1% - 2.0%).
% ==========================================================================

%% ==========================================================================
% 1. ENVIRONMENT SETUP
% ==========================================================================

clear; clc; close all;

addpath(fullfile(pwd, 'config'));
addpath(fullfile(pwd, 'models'));
addpath(fullfile(pwd, 'decoder'));

params = default_params();
rng(params.rng_seed);

loss_rates = [0.001, 0.005, 0.010, 0.015, 0.020];
num_tests = length(loss_rates);

rmse_theta_list  = zeros(1, num_tests);
final_drift_list = zeros(1, num_tests);

%% ==========================================================================
% 2. BASELINE TRAJECTORY & NOISE FLOOR
% ==========================================================================

[t, theta_true, ~] = trajectory_model(params);
[A_ideal, B_ideal, ~] = encoder_model(theta_true, params);
[A_pre_noisy, B_pre_noisy] = inject_micro_noise(A_ideal, B_ideal);

fprintf('\nRunning Sensitivity Analysis (Pulse Loss Sweep)...\n');

%% ==========================================================================
% 3. TEST LOOP
% ==========================================================================

for i = 1:num_tests
    rate = loss_rates(i);
    
    [A_noisy, B_noisy, ~] = inject_pulse_loss(A_pre_noisy, B_pre_noisy, rate);
    
    [pos_count, missing_count] = quadrature_decoder_x4(A_noisy, B_noisy);
    
    theta_estimated = (pos_count * 2*pi) / (params.PPR * 4);
    theta_comp = position_compensator(theta_estimated, missing_count, params.PPR);
    
    err_comp = theta_true(:) - theta_comp(:);
    
    rmse_theta_list(i)  = sqrt(mean(err_comp.^2));
    final_drift_list(i) = abs(err_comp(end));
end

%% ==========================================================================
% 4. RESULTS REPORT
% ==========================================================================

fprintf('\n=======================================\n');
fprintf('Loss Rate | RMSE Theta | Final Drift\n');
fprintf('---------------------------------------\n');
for i = 1:num_tests
    fprintf('  %4.1f %%  |  %8.4f  |   %8.4f\n', ...
        loss_rates(i)*100, rmse_theta_list(i), final_drift_list(i));
end
fprintf('=======================================\n\n');

%% ==========================================================================
% 5. VISUALIZATION
% ==========================================================================

fig_sens = figure('Name', 'Sensitivity Analysis', ...
    'Position', [150, 150, 950, 450]);

subplot(1,2,1);
plot(loss_rates * 100, rmse_theta_list, '-ob', 'LineWidth', 2);
title('Position RMSE vs Pulse Loss Rate');
xlabel('Pulse Loss (%)'); ylabel('RMSE \theta (rad)'); grid on;

subplot(1,2,2);
plot(loss_rates * 100, final_drift_list, '-or', 'LineWidth', 2);
title('Final Drift vs Pulse Loss Rate');
xlabel('Pulse Loss (%)'); ylabel('Final Drift (rad)'); grid on;

%% ==========================================================================
% 6. SAVE FIGURE
% ==========================================================================

if ~exist('results/figure/sensitivity', 'dir')
    mkdir('results/figure/sensitivity');
end
saveas(fig_sens, 'results/figure/sensitivity/sensitivity analysis.png');
fprintf('>> Figure saved: results/figure/sensitivity/sensitivity analysis.png\n');