function [omega_M, omega_T, omega_Hybrid] = speed_estimator(pos_count, t, PPR, error_flag)
    ...
    % =========================================================
    % 1. M-METHOD (Bản gốc thô - Giữ nguyên đặc tính)
    ...
    % =========================================================
    % 2. T-METHOD (Từ chối mẫu lỗi + Chặn Outlier đã được fix)
    % =========================================================
    last_time = t(1);
            ...
            if pos_count(i) ~= last_cnt
                ...
                if delta_t >= Ts
                    ...
                    if i > 1 && abs(tmp_omega - omega_T(i-1)) > 40
                        ...
                        % ---- ĐÂY LÀ 2 DÒNG CODE VÁ LỖI CỰC KỲ QUAN TRỌNG ----
                        % Vẫn phải cập nhật mốc thời gian để hệ thống không bị kẹt ở quá khứ
                        last_time = t(i);       
    ...
    % =========================================================
    % 3. HYBRID ESTIMATOR (Adaptive Fusion)
    % =========================================================
    for i = 1:N
        abs_M = abs(omega_M(i));
    ...
    % =========================================================
    % 4. BỘ LỌC CUỐI (Chỉ áp dụng lọc nhẹ cho Hybrid)
    % =========================================================
    window_ma = round(0.005 / Ts); % Cửa sổ lọc nhẹ 5ms cho Hybrid
    if window_ma < 1; window_ma = 1; end
